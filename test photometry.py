# -*- coding: utf-8 -*-
"""
Created on Fri Jun 19 11:47:51 2026
...
path = r"D:\headers\alignment\calib 2005-05-08 Imaging Ceres8_K expt-1.0 OP3 FULL OP6 H HD DR S13"
variables.update(csv_to_dict(os.path.join(path, "Ephemcc 2005-05-09T02_43_27.378.csv")))
variables.update(csv_to_dict(os.path.join(path, "Ephemph 2005-05-09T02_43_27.378.csv")))
fits_path = os.path.join(path, "2005-05-09T02_36_40.993 - 2005-05-09T02_50_47.882.fits")
...
def geo_to_pixel(lon_deg, lat_deg):
    ...
    Y_body = np.cos(lat_rad) * np.cos(lon_rad - lambda_0)
    Z_body = np.sin(lat_rad)
    

    x_prime = X_body
    y_prime = Z_body * np.cos(phi_0) - Y_body * np.sin(phi_0)
    z_prime = Z_body * np.sin(phi_0) + Y_body * np.cos(phi_0)
    ...
    y_disk = x_prime * np.sin(
        P) + y_prime * np.cos(P)
    

    px = x0 + x_disk * R_pixel
    py = y0 + y_disk * R_pixel
...
plt.plot(x0 + R_pixel*np.cos(theta), y0 + R_pixel*np.sin(theta), 'g--', alpha=0.7, label='Ceres Limb')
...
mask_emis = theta_deg < angle_eff_deg
img_masked = np.where(mask_emis, img_lonlat, np.nan)
...
mask_inci = theta_deg1 < angle_eff_deg
img_masked = np.where(mask_inci, img_lonlat, np.nan)
...
map_values = img_lonlat[mask_map]

map_values = map_values[~np.isnan(map_values)]


map_mean = np.mean(map_values)
map_sum = np.sum(map_values)
map_median = np.median(map_values)


theta_pixel = np.full(img.shape, np.nan)


phi_pix = lat_map * deg_rad
lam_pix = lon_map * deg_rad


cos_theta_pix = (
    np.sin(phi_pix) * np.sin(phi0)
    +
    np.cos(phi_pix) * np.cos(phi0)
    * np.cos(lam_pix-lam0)
)


theta_pixel = np.arccos(cos_theta_pix)


theta_pixel_deg = theta_pixel / deg_rad


mask_original = (
    disk_mask
    &
    (theta_pixel_deg < angle)
)



original_values = img[mask_original]
...
output_txt = os.path.join(path2, "photometry_comparison.txt")

with open(output_txt, "w", encoding="utf-8") as f:

    f.write("========== Photometry comparison ==========\n\n")

    f.write("Original pixel:\n")
    f.write(f"N = {len(original_values)}\n")
    f.write(f"Mean = {original_mean}\n")
    f.write(f"Median = {original_median}\n")
    f.write(f"Sum = {original_sum}\n\n")


    f.write("Lon-Lat map:\n")
    f.write(f"N = {len(map_values)}\n")
    f.write(f"Mean = {map_mean}\n")
    f.write(f"Median = {map_median}\n")
    f.write(f"Sum = {map_sum}\n\n")


    f.write("Difference:\n")
    f.write(
        f"Mean difference (%) = "
        f"{(map_mean-original_mean)/original_mean*100}\n"
    )

    f.write(
        f"Median difference (%) = "
        f"{(map_median-original_median)/original_median*100}\n"
    )
